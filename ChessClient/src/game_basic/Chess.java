package game_basic;		//client

import java.awt.Color;

import java.util.Scanner;

import added_Function.Client;
...
public class Chess {
	...
	public static boolean whiteCastling(String str, Scanner sc,int input) {
		...
		if(str.equals("castling") || str.equals("Castling")  || str.equals("CASTLING") ) {
			...
			/////////////////queensidecastling/////////////////////////////////
			if(input ==1) {
				if(Locate.location[1][0].isEmpty() && Locate.location[2][0].isEmpty() && Locate.location[3][0].isEmpty()) {
					if(Unit.rook[0].isFirst()) {
	...
	public static void whiteTurn(Scanner sc,GUIboard g,Client client) {
		String str;
		...
		while(true) {
			...
			str = catchCoordinate(client.getSender(),false);
			boolean isWhite = true;
			//castled = whiteCastling(str, sc);
		...
		System.out.println("type the x and y of destination");
			...
			while (true) {
				boolean result = instructMove(a, b, sc,client.getSender(), false);
	...
	public static void blackTurn( Scanner sc,GUIboard g, Client client) {
		String str2;
		boolean castled = false;
		int a = 8;
		int b = 8;
		...
		while(true) {
			...
			System.out.println("a unit you want to control");
			str2 = catchCoordinate(client.getSender(),true);
		...
		if(!castled && !str2.equals("continue")) {
			System.out.println("type the x and y of destination");
			while (true) {
				if(instructMove(a, b, sc, client.getSender(), true)) break;		//무빙 성공하면 입력받음.
				else System.out.println("sorry, your destination is invalid. retype.");
			}
		}
	}
	...
	public static void gameStart(Scanner sc) {
		GUIboard g =new GUIboard("Chess 1.0 Black");
		...
		while(true) {
			Locate.saveLocate();
			if(Chess.turn.equals("TRUE"))cleaner(client);								//성공시 위치세이브..
			whiteTurn(sc,g,client);
			if(Chess.blackCheck()) {
				Locate.loadLocate();
				g.boardSet();
			...
			Locate.saveLocate();
			if(Chess.turn.equals("FALSE"))cleaner(client);	
			while(true) {
				blackTurn(sc,g ,client);
				if(Chess.whiteCheck()) {
					System.out.println("Your king will be dead. retype.(Check)");
					Locate.loadLocate();
					g.boardSet();
	...
	public static void cleaner(Client client) {
		client.getReader().getBox().setStr("empty");
		client.getSender().getBox().setStr("empty");
		Chess.setStr("empty");
		Chess.turn = "no";
	}
